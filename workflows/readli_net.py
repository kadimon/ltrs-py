...
from workflow_base import BaseLitresPartnersWorkflow, BaseLivelibWorkflow
from interfaces import InputLivelibBook, InputLitresPartnersBook, Output, WorkerLabels
from db import DbSamizdatPrisma, save_book_mongo
    ...
    async def task(cls, input: InputLivelibBook, page: Page) -> Output:
        ...
        async with DbSamizdatPrisma() as db:
            ...
            if annotation := await page.inner_text('.seo__content'):
                book['annotation'] = re.sub(r'^АННОТАЦИЯ\n', '', annotation.strip()).strip()
...
if __name__ == '__main__':
    task_type = input('Выберите скрапер: партрерский или самиздат (P/S):')
    if task_type.lower() == 'P':
        ReadliNet.run_sync()

        ReadliNet.debug_sync('https://readli.net/kogda-nitsshe-plakal/')

    elif task_type.lower() == 's':
        ...
        ReadliNetItem.debug_sync('https://readli.net/shpik-si/')
        ReadliNetItem.debug_sync('https://readli.net/shurik-1970/')

    else:
        print(f'Опции "{task_type}" не существует!!!')
