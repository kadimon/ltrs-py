...
class MangalibItem(BaseLivelibWorkflow):
    name = 'livelib-mangalib-item'
    event = 'livelib:mangalib-item'
    site = 'mangalib.me'
    ...
    async def task(cls, input: InputLivelibBook, page: Page) -> Output:
        ...
        # JS: await page.waitForSelector("h1")
        await page.wait_for_selector("h1")

        # JS: if (response.status() == 404 || page.url() == "https://mangalib.me/404")
        if resp.status == 404 or page.url == "https://mangalib.me/404":
        ...
        # JS: await page.waitForResponse(resp => resp.url().includes("/stats?bookmarks=true&rating=true"))
        try:
            await page.wait_for_response(
                lambda r: "/stats?bookmarks=true&rating=true" in r.url,
                timeout=15000,
            )
        except Exception:
            pass
        ...
        async with DbSamizdatPrisma() as db:
            ...
            # --- Вспомогательная функция для полей вида "Автор / Издатель / Художник / Переводчик" ---
            # JS: x.map(...) -> {name, url} и join(", ")
            ...
            # Title (только при создании, как в JS: внутри if !checkBookExist)
            if not await db.check_book_exist(page.url):
                title_locator = page.locator("h1 > span")
                if await title_locator.count() > 0:
                    if title := (await title_locator.first.text_content() or "").strip():
                        book['title'] = title
            ...
            # Translators
            # JS: div.team-item__name
            translators_locator = page.locator("div.team-item__name")
...
class MangalibListing(BaseLivelibWorkflow):
    name = 'livelib-mangalib-listing'
    event = 'livelib:mangalib-listing'
    site = 'mangalib.me'

    input = InputLivelibBook
    output = Output
    item_wf = MangalibItem
    ...
    start_urls = ["https://api2.mangalib.me/api/manga?site_id[]=1&page=1"]
    ...
    async def task(cls, input: InputLivelibBook, page: Page) -> Output:
        ...
        if next_page_url:
            if page_match := re.search(r'page=(\d+)', next_page_url):
                ...
                next_url = f"https://api2.mangalib.me/api/manga?site_id[]=1&page={page_num}"
                if await cls.crawl(next_url, input.task_id):
                    stats['new-page-links'] += 1

        # Книги
        # JS: data["data"].forEach(i => "https://mangalib.me/ru/manga/" + i["slug_url"])
        for i in data.get("data", []):
            book_url = "https://mangalib.me/ru/manga/" + i["slug_url"]
            if await MangalibItem.crawl(book_url, input.task_id):
